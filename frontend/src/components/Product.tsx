...
import {advanceProduct, deleteProduct, getAllProducts, updateProduct} from "../service/apiService";
...
export default function Product({infos, update} : ProductProps){
    ...
    const statusRevert = () => {
        if (infos.status ==='OPEN'){
            deleteProduct(infos.id, token).then(()=>
                getAllProducts(token)
                .then(data => update(data)))
        } else if (infos.status ==='DONE') {
            infos.status='IN_PROGRESS'
            updateProduct(infos.id,infos,token).then(()=>
                getAllProducts(token)
                    .then(data => update(data)))
        } else {
            infos.status='OPEN'
    ...
    const nextStatus = () => {
        advanceProduct(infos, token).then(()=>
            getAllProducts(token)
                .then(data => update(data)))
    }
